/**
 * Radix Colors palette data.
 *
 * Extracted from @radix-ui/colors source (src/light.ts and src/dark.ts).
 * Each palette has 12 steps (1–12), where 1 is the lightest and 12 is the darkest.
 *
 * Format: { paletteName: { light: [step1..step12], dark: [step1..step12] } }
 */
...
export const RADIX_PALETTES: Record<string, RadixPalette> = {
  gray: {
  ...
  // Note: Radix also has "mint" but we already use "mint" as a theme name.
  // Skip the Radix mint palette to avoid confusion.
};
