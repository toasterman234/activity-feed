  ...
  return (
    <div className="space-y-4 pb-8">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Initiatives</h2>
          <p className="mt-0.5 max-w-xl text-[11px] leading-relaxed text-zinc-500 dark:text-zinc-400">
            Tracked work with PLAN proof. Tap a title for links, contract, findings, and timeline.
            <span className="font-medium text-zinc-700 dark:text-zinc-300"> Promote </span>
            admits shipped in the graph when checks pass — it does not rewrite PLAN files.
          </p>
        </div>
        <button
          onClick={() => {
              ...
              <ul className="space-y-2">
                {filteredRows.map(({ init, map, gateOk, openFindings, canPromote, kind }) => {
                  ...
                  return (
                    <li
                      key={key}
                      className="rounded-xl border border-zinc-200 bg-card p-3 dark:border-zinc-800 dark:bg-zinc-900"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <div className="text-sm font-medium text-zinc-900 dark:text-zinc-100">
                            {gateOk ? "✓" : "✗"}{" "}
                            {init ? (
                              <Link href={`/channels/continuity/${init.id}`} className="underline-offset-2 hover:underline">
                                {title}
                              </Link>
                            ) : (
                              title
                          ...
                          {init && (
                            <Link
                              href={`/channels/continuity/${init.id}`}
