...
export default function MoneyFlowContent() {
  ...
  useEffect(() => {
    const fetches = [
      { key: "risk", url: "/api/fred/risk-appetite" },
      { key: "sectors", url: "/api/finance/sector-rs" },
      { key: "cot", url: "/api/finance/cot" },
    ];
    const results = fetches.map(({ key, url }) =>
      fetch(url, { signal: AbortSignal.timeout(15_000) })
        .then(async (r) => {
          if (!r.ok) throw new Error(`HTTP ${r.status}`);
          return { key, data: await r.json() };
        })
        .catch((e) => ({ key, error: e instanceof Error ? e.message : String(e) })),
    );
    Promise.all(results).then((outcomes) => {
      ...
      for (const o of outcomes) {
        if ("error" in o) {
          errs[o.key] = o.error;
        } else if (o.key === "risk") setRisk(o.data);
        else if (o.key === "sectors") setSectors(o.data);
        else if (o.key === "cot") setCot(o.data);
      }
      setErrors(errs);
      setLoading(false);
    });
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Sector Rotation ── */}
      {errors.sectors ? (
        <ErrorCard message={errors.sectors} />
      ) : (
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
