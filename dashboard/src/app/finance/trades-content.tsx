...
function getTradeShape(): Promise<ShapeMaterialization> {
  return acquireShape("trades", () => client.shape(TRADES_SHAPE));
}
...
export default function TradesContent() {
  ...
  useEffect(() => {
    let alive = true;
    getTradeShape().then((s) => { if (alive) setShape(s); });
    return () => { alive = false; releaseShape("trades"); };
  }, []);
  ...
  return <TradesView shape={shape} />;
}

function TradesView({ shape }: { shape: ShapeMaterialization }) {
            ...
            <tbody>
              {pageRows.map((t) => (
                <tr key={t.trade_id} className="border-b border-zinc-50 dark:border-zinc-800/50">
                  ...
                  <td className="px-6 py-2.5 font-medium text-zinc-800 dark:text-zinc-200">{t.symbol || "—"}{t.is_option && <span className="ml-1 text-xs text-amber-500">OPT</span>}</td>
